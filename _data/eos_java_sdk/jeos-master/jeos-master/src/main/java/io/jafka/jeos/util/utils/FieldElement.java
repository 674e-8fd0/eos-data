...
public class FieldElement {
	...
	public FieldElement(BigInteger q, BigInteger x) throws Exception {
		this.x = x;
		if (x.compareTo(q) >= 0) {
			throw new Exception("x value too large in field element");
		}
		this.q = q;
	}

	public FieldElement add(FieldElement b) throws Exception {
		return new FieldElement(q, x.add(b.toBigInteger()).mod(q));
	}

	public FieldElement subtract(FieldElement b) throws Exception {
		return new FieldElement(q, x.subtract(b.toBigInteger()).mod(q));
	}

	public FieldElement multiply(FieldElement b) throws Exception {
		return new FieldElement(q, x.multiply(b.toBigInteger()).mod(q));
	}

	public FieldElement divide(FieldElement b) throws Exception {
		return new FieldElement(q, x.multiply(b.toBigInteger().modInverse(q)).mod(q));
	}

	public FieldElement negate() throws Exception {
		return new FieldElement(q, x.negate().mod(q));
	}

	public FieldElement square() throws Exception {
		return new FieldElement(q, x.multiply(x).mod(q));
	}

	public FieldElement invert() throws Exception {
	...
	public FieldElement sqrt() throws Exception {
		if (!q.testBit(0)) {
